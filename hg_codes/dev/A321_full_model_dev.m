...
%     n=calculateGustLoadFactor(TrimLoadcase1,Aircraft);
    ...
    run_folder = [
        'D:\MATLAB_workspace\Git_ALENA\ALENA_git_v1\ALENA-master\hg_codes\results\nonlinear1']; %[-], folder for exporting the NASTRAN model
...
nonlinear = NastranMethods1.writeTrimFile(Aircraft, TrimLoadcase1, MassCases,run_folder,'DatFilename','A321_cruise_g');
...
%    
%     delete(strcat(run_folder, '\A321_cruise_g*.xdb.*'));
%     delete(strcat(run_folder, '\A321_cruise_g*.h5.*'));
%     delete(strcat(run_folder, '\A321_cruise_g*.log.*'));
%     delete(strcat(run_folder, '\A321_cruise_g*.f06.*'));
%     delete(strcat(run_folder, '\A321_cruise_g*.f04.*'));
%     delete(strcat(run_folder, '\A321_cruise_g*.xdb'));
%     delete(strcat(run_folder, '\A321_cruise_g*.h5'));
%     delete(strcat(run_folder, '\A321_cruise_g*.log'));
%     delete(strcat(run_folder, '\A321_cruise_g*.f06'));
...
% 
% res_aeroF = mni.result.f06(strcat(run_folder,'/a321_cruise_g.f06')).read_aeroF;
% % 
% FlightPoint=awi.model.FlightPoint;
...
% % CL CD
% 
% Drag_wing = sin(WJ(idx)).*res_aeroF.aeroFz(idx)';
% Drag_conn = sin(WJ(idx_c)).*res_aeroF.aeroFz(idx_c)';
% Drag_tail = sin(WJ(idx_t)).*res_aeroF.aeroFz(idx_t)';
% 
% % t=atan(WJ/5);
...
% 
%     
%     Trim1_res=NastranMethods1.extractNastranResults(strcat(run_folder,'\A321_cruise_g.f06'),'ReadF06',true,'ReadHDF5',false);
