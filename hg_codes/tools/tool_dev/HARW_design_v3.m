...
All_eta=[1];
Wingbox_Weight_Recard=zeros(1,length(All_eta));
...
Res_Name=strcat('C:\Git\A321_sizing\hg_codes\Data\sizing_correction\A126\AR10\Res_AR10_Eta_',num2str(All_eta(i)*100));

% Update Initial Guess
% prevous_result=load('D22_results\Res_AR22_Eta_100_Model');

% prevous_result=load(strcat('C:\Git\A321_sizing\hg_codes\Data\Sizing_with_upated_hinge_lock\A160\AR19\Res_AR19_Eta_',num2str(All_eta(i)*100),'_Model'));
prevous_result=load(strcat('D22_results\Res_AR10_Eta_',num2str(All_eta(i)*100),'_Model'));
...
run_folder = ['C:\Git\A321_sizing\hg_codes\results\AR_Eta_Study']; %[-], folder for exporting the NASTRAN model
 ...
 %% Material properties : Al7075
   ...
   %% Sizing Analysis 
    ...
    [FEM_model,CDi,CD0,CL,k,Distribution,Load_distribution, Displacements_Res]=Static_Trim_v1(Param, run_folder, 'Load_Factor',1,'File_Name','Jig_twist_1g','Hinge_Lock','on');
    ...
    while max(cond_set1)>1.05 || min(cond_set2)<0.95
        ...
        while max(Discrepancy_L)>0.06
            ...
            [FEM_model,CDi,CD0,CL,k,Distribution,~, Displacements_Res]=Static_Trim_v1(Param, run_folder, 'Load_Factor',1,'File_Name','Jig_twist_1g','Hinge_Lock','on');
