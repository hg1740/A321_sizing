function [CDi,CD0,CL,k,Distribution,Load_distribution, Displacements_Res]=Static_trim(Param, Load_facor, Name, Lock, run_folder)
    ...
    TrimLoadcase.LoadFactor = Load_facor;
    ...
    trimFile = NastranMethods.writeTrimFile(Aircraft, TrimLoadcase,...
        MassCases,run_folder,'DatFilename',Name,...
        'ExtraCards',aelink);
...
%     delete(strcat(run_folder, '\*.op4'));
    ...
    Trim1_res=NastranMethods.extractNastranResults(strcat(run_folder,'\',Name,'.f06'),'ReadF06',true,'ReadHDF5',false);
    ...
    structural_nodes = h5read(strcat(run_folder,'\',Name,'.h5'),'/NASTRAN/INPUT/NODE/GRID');
    Displacements=h5read(strcat(run_folder,'\',Name,'.h5'),'/NASTRAN/RESULT/NODAL//DISPLACEMENT');
...
%         Y_FWT=X_FWT.X(2,:)';
    ...
    res_aeroF = mni.result.f06(strcat(run_folder,'/',Name,'.f06')).read_aeroF;
    ...
    Distribution.Y=Y_all;
    Distribution.Y_conn=Y_conn;
    Distribution.Y_wing1=Y_wing1;
    Distribution.Y_wing2=Y_wing2;
    
    if ~isempty(FWT_)
        Distribution.Y_fwt=Y_fwt;
    end
         
    Distribution.WJ=wj';
    Distribution.Alphai=alphai';
    Distribution.Chord_Length=Chord_Lengths';
    Distribution.CL_var=CL_var';
    Distribution.CDi_var=CDi_var';
    Distribution.Lift_var=Lift_all';
    
    Distribution.Panel_width=panel_width_all;
