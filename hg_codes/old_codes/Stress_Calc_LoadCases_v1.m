%% SOL 144 + SOL 146
function [Von_skn, Von_spr, sigmab_skn, taub_skn, sigma_pp, sigma_strg, sigma_crip, sigma_col]=Stress_Calc_LoadCases_v1(x,Semi_span,Root_chord,LE_sweep,TE_sweep,BeamLoc,Fuselage_length, Wing_position, Horizontal_tail_position, Vertical_tail_position, Secondary_mass,Fuel_mass,Fuselage_total_mass)
...
%     getFlightPointData(FlightPoint,'ISA');
    ...
    %% Loadcase 2: cruising altitude -g dive
...
%     delete('D:\MATLAB_workspace\ALENA-master\ALENA-master\hg_codes\Sizing_analysis\Result\A320_half_model_SOL144*.*')
    ...
    delete(strcat(run_folder, '\gust_analysis*.xdb'));
    delete(strcat(run_folder, '\gust_analysis*.h5'));
    delete(strcat(run_folder, '\gust_analysis*.log'));
    delete(strcat(run_folder, '\gust_analysis*.f06'));
    delete(strcat(run_folder, '\gust_analysis*.f04'));
    ...
    % maximum internal load distribution: 
    % 1g + max. Positive delta 
    % -g + max. negative delta
    
    %% Internal Load Summery 
    
    % Load case 1: 2.5 g pull up
    ...
    % Load case 2:  - g dive
    Moment_P1_Loadcase2=abs(Trim2.M_P1); % internal load M1 for - g dive
    Moment_P2_Loadcase2=abs(Trim2.M_P2); % internal load M2 for - g dive
    Torque_Loadcase2=abs(Trim2.T);       % internal load T for - g dive
    Shear_P1_Loadcase2=abs(Trim2.S_P1);
    Shear_P2_Loadcase2=abs(Trim2.S_P2);
    
    % Load case 3:  + g + positive delta
    Moment_P1_Loadcase3=abs(Trim1.M_P1)/2.5; 
    Moment_P2_Loadcase3=abs(Trim1.M_P2)/2.5 + [max_M; 0]'; 
    Torque_Loadcase3=abs(Trim1.T)/2.5 + [max_T; 0]'; 
    Shear_P1_Loadcase3=abs(Trim1.S_P1)/2.5 + [max_V; 0]';
    Shear_P2_Loadcase3=abs(Trim1.S_P2)/2.5 + [max_V; 0]';
    
    % Load case 4:  - g + negative delta
    Moment_P1_Loadcase4=abs(Trim2.M_P1); 
    Moment_P2_Loadcase4=abs(Trim2.M_P2) + abs([min_M; 0]'); 
    Torque_Loadcase4=abs(Trim2.T) + abs([min_T; 0]');  
    Shear_P1_Loadcase4=abs(Trim2.S_P1) + abs([min_V; 0]');
    Shear_P2_Loadcase4=abs(Trim2.S_P2) + abs([min_V; 0]');
    

    % Maximum forces used for sizing
    M_P1=max([Moment_P1_Loadcase1; Moment_P1_Loadcase2; Moment_P1_Loadcase3; Moment_P1_Loadcase4]);
    M_P2=max([Moment_P2_Loadcase1; Moment_P2_Loadcase2; Moment_P2_Loadcase3; Moment_P2_Loadcase4]);
    T=max([Torque_Loadcase1; Torque_Loadcase2; Torque_Loadcase3; Torque_Loadcase4]);
    
    S_P1=max([Shear_P1_Loadcase1; Shear_P1_Loadcase2; Shear_P1_Loadcase3; Shear_P1_Loadcase4]);
    S_P2=max([Shear_P2_Loadcase1; Shear_P2_Loadcase2; Shear_P2_Loadcase3; Shear_P2_Loadcase4]); 
    
    %% Figures
    
%     figure 
%     plot(Y-2,Shear_P2_Loadcase1,'bs','MarkerFaceColor','b')
%     hold on
%     plot(Y-2,Shear_P2_Loadcase2,'gs','MarkerFaceColor','g')
%     hold on
%     plot(Y-2,Shear_P2_Loadcase3,'ks','MarkerFaceColor','k')
%     hold on
%     plot(Y-2,Shear_P2_Loadcase4,'ms','MarkerFaceColor','m')
%     hold on
%     plot(Y-2,S_P2,'r-','LineWidth',1.2)
%     
%     legend('2.5 g','-g','g + 1MC','-g + 1MC','Sizing','Interpreter','latex','FontSize',10)
...
% 
%     figure
%     plot(Y-2,Moment_P2_Loadcase1,'bs','MarkerFaceColor','b')
%     hold on
%     plot(Y-2,Moment_P2_Loadcase2,'gs','MarkerFaceColor','g')
%     hold on
%     plot(Y-2,Moment_P2_Loadcase3,'ks','MarkerFaceColor','k')
%     hold on
%     plot(Y-2,Moment_P2_Loadcase4,'ms','MarkerFaceColor','m')
%     hold on
%     plot(Y-2,M_P2,'r-','LineWidth',1.2)
%     
%     legend('2.5 g','-g','g + 1MC','-g + 1MC','Sizing','Interpreter','latex','FontSize',10)
%     xlabel('Span distance (m)','FontSize',12,'Interpreter','latex')
...
%     
%     figure
%     plot(Y-2,Torque_Loadcase1,'bs','MarkerFaceColor','b')
%     hold on
%     plot(Y-2,Torque_Loadcase2,'gs','MarkerFaceColor','g')
%     hold on
%     plot(Y-2,Torque_Loadcase3,'ks','MarkerFaceColor','k')
%     hold on
%     plot(Y-2,Torque_Loadcase4,'ms','MarkerFaceColor','m')
%     hold on
%     plot(Y-2,T,'r-','LineWidth',1.2)
%     
%     legend('2.5 g','-g','g + 1MC','-g + 1MC','Sizing','Interpreter','latex','FontSize',10)
%     xlabel('Span distance (m)','FontSize',12,'Interpreter','latex')
...
%     
%     figure 
%     plot(Y(1:24)-2,max_V,'rs','MarkerFaceColor','r')
%     hold on 
%     plot(Y(1:24)-2,min_V,'bs','MarkerFaceColor','b')
%     
%     xlabel('Span distance (m)','FontSize',12,'Interpreter','latex')
%     ylabel('$\Delta$ Vertical shear (N)','FontSize',12,'Interpreter','latex')
%     set(gcf,'color','w')
%     legend('Positive Max.','Negative Max.','Interpreter','latex','FontSize',12)
%     
%     figure
%     plot(Y(1:24)-2,max_M,'rs','MarkerFaceColor','r')
%     hold on
%     plot(Y(1:24)-2,min_M,'bs','MarkerFaceColor','b')
%     
%     xlabel('Span distance (m)','FontSize',12,'Interpreter','latex')
...
%     
%     figure
%     plot(Y(1:24)-2,max_T,'rs','MarkerFaceColor','r')
%     hold on
%     plot(Y(1:24)-2,min_T,'bs','MarkerFaceColor','b')
%     
%     xlabel('Span distance (m)','FontSize',12,'Interpreter','latex')
